...
const AddGame = () => {
  ...
  const handleValueSelect = (username: string) => {
    ...
    if (selectedPlayer) {
      ...
      form.setFieldValue("players", [
        ...form.values.players,
        { _id: selectedPlayer._id, score: "", username: selectedPlayer.username, 
          team: "", email: selectedPlayer.email, image: selectedPlayer.image, 
          created_at: selectedPlayer.created_at, total_matches: selectedPlayer.total_matches, 
          wins: selectedPlayer.wins, longest_winstreak: selectedPlayer.longest_winstreak },
      ]);
    }
  };
  ...
  const handleValueRemove = (username: string) => {
    form.setFieldValue(
      "players",
      form.values.players.filter((player) => player.username !== username)
    );
  };
  ...
  const values = form.values.players.map((player) => (
    <Pill
      key={player.username}
      withRemoveButton
      onRemove={() => handleValueRemove(player.username)}
  ...
  const options = players
    ...
    .map((player) => (
      <Combobox.Option
        ...
        key={player.username}
        active={form.values.players.some((p) => p.username === player.username)}
      >
        <Group gap="sm">
          {form.values.players.some((p) => p.username === player.username) ? <CheckIcon size={12} /> : null}
  ...
  const handleSubmit = async (values: typeof form.values) => {
    ...
    if (Array.isArray(values.players)) {
      values.players.forEach((player, index) => {
        ...
        data.append(`players[${index}][name]`, player.username);
                        ...
                        <PillsInput.Field
                          ...
                          onKeyDown={(event) => {
                            if (event.key === "Backspace" && search.length === 0) {
                              ...
                              handleValueRemove(form.values.players[form.values.players.length - 1]?.username);
                            }
                          }}
                        />
                      </Combobox.EventsTarget>
